...
if (isset($_POST['form1'])) {
    ...
    if ($captcha_status == 1 && (!isset($_POST['g-recaptcha-response']) || empty($_POST['g-recaptcha-response']))){
            $valid = 0;
        ...
        if($captcha_status == 1){
            ...
            if($captcha_status == 1 && $recaptchaStatus) $valid = 1;
        }
    }
    ...
    if($valid == 1) {
		...
		$statement = $pdo->prepare("INSERT INTO tbl_cart(cust_id,p_id,size_id,size_name,color_id,color_name,cust_val,p_type,p_qty,p_current_price,p_name,p_featured_photo)
		                           VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
		$statement->execute(array($cust_id,$cart_p_id,$cart_size_id,$cart_size_name,$cart_color_id,$cart_color_name,$cart_cust_val,$cart_p_type,$cart_p_qty,$cart_p_current_price,$cart_p_name,$cart_p_featured_photo));
